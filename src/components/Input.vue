<template>
  <div class="vue-treeselect__input-container" v-bind="containerProps">
    ...
    />
    <div v-if="needAutoSize" ref="sizerRef" class="vue-treeselect__sizer">
      {{ inputValue }}
    </div>
  </div>
</template>
...
import { KEY_CODES, MIN_INPUT_WIDTH } from "../constants";
...
const searchable = computed(
  () => (instanceNonNull.value as any).searchable !== false,
);
...
const required = computed(
  () => (instanceNonNull.value as any).required || false,
);
const hasValue = computed(() => instanceNonNull.value.hasValue);
const backspaceRemoves = computed(
  () => (instanceNonNull.value as any).backspaceRemoves !== false,
);
const deleteRemoves = computed(
  () => (instanceNonNull.value as any).deleteRemoves !== false,
);
const disableBranchNodes = computed(
  () => (instanceNonNull.value as any).disableBranchNodes,
);
...
function onKeyDown(evt: KeyboardEvent) {
  ...
  if (
    !instanceNonNull.value.menuIsOpen &&
  ...
  switch (key) {
    ...
    case KEY_CODES.ARROW_LEFT: {
      const currentId = instanceNonNull.value.currentHighlightedOptionId;
      const current =
        currentId !== null ? instanceNonNull.value.getNode(currentId) : null;
      if (
        current &&
        current.isBranch &&
        (instanceNonNull.value as any).shouldExpand?.(current)
      ) {
        evt.preventDefault();
        ...
        evt.preventDefault();
        if (
          (instanceNonNull.value as any).setCurrentHighlightedOption &&
          current.parentNode
        ) {
          (instanceNonNull.value as any).setCurrentHighlightedOption(
            current.parentNode,
...
// Watchers
watch(
  () => instanceNonNull.value.trigger.searchQuery,
  (newValue) => {
