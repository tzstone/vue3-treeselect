...
import { setupResizeAndScrollEventListeners, watchSize } from "../utils";
...
const remoteSearchEntry = computed(() =>
  instance.value!.getRemoteSearchEntry(),
);

// Computed: Should show search prompt tip
const shouldShowSearchPromptTip = computed(
  () =>
    instance.value!.trigger.searchQuery === "" &&
    !(instance.value as any).defaultOptions,
);

// Computed: Should show no results tip (async search)
const shouldShowAsyncSearchNoResultsTip = computed(
  () =>
    !shouldShowSearchPromptTip.value &&
    remoteSearchEntry.value.isLoaded &&
    remoteSearchEntry.value.options.length === 0,
);
...
const hasNoOptions = computed(
  () =>
    instance.value!.rootOptionsStates.isLoaded &&
    instance.value!.forest.normalizedOptions.length === 0,
);
...
function adjustMenuOpenDirection() {
  ...
  if (!isControlInViewport) {
    ...
    instance.value!.menu.placement =
      directionMap[instance.value!.openDirection] || "bottom";
    ...
    <transition name="vue-treeselect__menu--transition">
      ...
      >
        ...
        <template v-if="instance!.async">
          <!-- Search prompt tip -->
          <Tip
            v-if="shouldShowSearchPromptTip"
            type="search-prompt"
            icon="warning"
          >
          ...
          <!-- Loading tip -->
          <Tip
            v-else-if="remoteSearchEntry.isLoading"
            type="loading"
            icon="loader"
          >
          ...
          <Tip
            v-else-if="remoteSearchEntry.loadingError"
            type="error"
            icon="error"
          >
          ...
          <!-- No results tip -->
          <Tip
            v-else-if="shouldShowAsyncSearchNoResultsTip"
            type="no-results"
            icon="warning"
          >
        ...
        <template v-else-if="instance!.localSearch.active">
          <!-- Loading tip -->
          <Tip
            v-if="instance!.rootOptionsStates.isLoading"
            type="loading"
            icon="loader"
          >
            {{ instance!.loadingText }}
          ...
          <Tip
            v-else-if="instance!.rootOptionsStates.loadingError"
            type="error"
            icon="error"
          >
          ...
          <!-- No results tip -->
          <Tip
            v-else-if="shouldShowLocalSearchNoResultsTip"
            type="no-results"
            icon="warning"
          >
            {{ instance!.noResultsText }}
