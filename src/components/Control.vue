<template>
  ...
  >
    <div class="vue-treeselect__value-container">
      ...
      <MultiValue v-if="!instanceNonNull.single" />

      <Placeholder v-if="!instanceNonNull.hasValue && instanceNonNull.single" />

      <Input v-if="instanceNonNull.searchable && instanceNonNull.single" />
    </div>
...
const shouldShowX = computed(() => {
  const hasUndisabledValue = instanceNonNull.value.selectedNodes.some(
    (node) => !node.isDisabled,
  );
