...
import type { TreeselectProps, TreeselectNode, TreeselectOption } from '../types'
import {
...
function createAsyncOptionsStates(): { isLoaded: boolean; isLoading: boolean; loadingError: string } {
  return {
    isLoaded: false,
    isLoading: false,
    loadingError: '',
  ...
  function handleLocalSearch(): void {
    ...
    traverseAllNodesDFS(node => {
      ...
      if (node.isMatched) {
        ...
        if (node.isLeaf) node.ancestors.forEach(ancestor => localSearch.countMap[ancestor.id][LEAF_DESCENDANTS]++)
        if (node.parentNode != null) {
          localSearch.countMap[node.parentNode.id][ALL_CHILDREN] += 1
          if (node.isLeaf) localSearch.countMap[node.parentNode.id][LEAF_CHILDREN] += 1
        }
      }
