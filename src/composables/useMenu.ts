...
import type { ForestState, MenuState, TriggerState, LocalSearchState } from './useForest'
...
export function useMenu(
  ...
  function setCurrentHighlightedOption(node: TreeselectNode, scroll = true): void {
    ...
    if (menu.isOpen && scroll) {
      const scrollToOption = () => {
        const $menu = getMenu()
        if (!$menu) return
        const $option = $menu.querySelector(`.vue-treeselect__option[data-id="${node.id}"]`) as HTMLElement | null
  ...
  function highlightPrevOption(): void {
    ...
    const prev = visibleOptionIds.value.indexOf(menu.current as string | number) - 1
  ...
  function highlightNextOption(): void {
    ...
    const next = visibleOptionIds.value.indexOf(menu.current as string | number) + 1
  ...
  const visibleOptionIds = computedVisibleOptionIds(
    forest, localSearch, shouldExpand, shouldShowOptionInMenu,
  )
  ...
  return {
    ...
    setupHandleClickOutside: (handler: (evt: MouseEvent) => void) => { handleClickOutside = handler },
  }
}
...
function computedHasVisibleOptions(
  visibleOptionIds: { get value(): (string | number)[] },
) {
  return {
    get value(): boolean {
