...
```bash
npm install @tzstone/vue3-treeselect
```
...
<script setup>
import { ref } from 'vue';
import Treeselect from '@tzstone/vue3-treeselect';
import '@tzstone/vue3-treeselect/dist/vue3-treeselect.css';
...
const loadOptions = ({ action, parentNode, callback }) => {
  if (action === 'LOAD_CHILDREN_OPTIONS') {
    ...
    setTimeout(() => {
      callback(null, {
        children: [{ id: `${parentNode.id}-1`, label: `Child of ${parentNode.label}` }],
      });
    }, 500);
  }
};
